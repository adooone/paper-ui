...
- `159b62a` — follow-up: Progress's viewBox height was fixed at 20 units regardless of the real rendered height (6px default), so the non-uniform stretch down to a short bar was squashing the rough.js jitter to near-invisibility. Height now draws at its real pixel value; sketchy edges are clearly visible even at 4px.
- `e77be1d` — replaced the old logo/favicon (a glossy multi-filter amber badge sharing no visual language with any component) with a mark generated from the library's actual `generateWobblyBlob`/`generateWobblyRing` algorithm (seed 12, wobble 0.35, blob+ring sharing one random sequence so the ring tracks the fill's contour) — the same fill+ring construction Button/Stamp use, filled with the amber accent gradient. SVG favicon added as primary (crisp at any size), PNG fallbacks rasterized via ImageMagick's built-in SVG support directly from the new source.
